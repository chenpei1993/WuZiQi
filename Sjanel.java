...
public class Sjanel extends JPanel {
	JFrame f;
	int BlackStep=0;
	int WhiteStep=0;
	boolean first=true;
					...
					if(!is){
						//if(first)//确定先手为黑
							if(BlackStep==WhiteStep){
								...
								Computerlocate=computer.ComputerStep(Blackpiece,Whitepiece);//交给AI 返回记录坐标的数组
								chessx=Computerlocate[0];
								chessy=Computerlocate[1];
